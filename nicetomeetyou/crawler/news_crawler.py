...
def get_news(pages: int = 2):

    url_base = "https://nba.udn.com/"
    url_news = "https://nba.udn.com/nba/news"
    headers = {
    ...
    def get_news_url_list(pages: int) -> list:
        ...
        r = session.get(url=url_news, headers=headers)
        ...
        while pages > 0:
            ...
            page_link_next = html.find('div > gonext > a[data-id=right]', first=True).attrs['href']

            r = session.get(url="https://nba.udn.com/" + page_link_next, headers=headers)
            pages += -1
    ...
    for url in news_url_list:
        ...
        post_title = story_body_content.find('h1.story_art_title', first=True).text
        post_image_url = story_body_content.find('figure.photo_center > a > img', first=True).attrs['data-src']
